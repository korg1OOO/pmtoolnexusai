 ...
 * Features:
 * - Strict color progress (red/orange/yellow/green)
 * - Animated flowing color wave background
 * - Clean node card with required fields
...
export const MindMapNode: React.FC<MindMapNodeProps> = ({
  ...
  // Calculate child count recursively
  ...
  // Get upcoming deadline (earliest due task)
  ...
  return (
    <div className={cn("group relative", className)}>
      ...
      >
        {/* Animated Color Wave Background (per skill requirement) */}
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="h-8 w-8 rounded-lg hover:bg-accent flex items-center justify-center text-muted-foreground hover:text-foreground"
                title="Add task"
              >
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <div className="relative h-14 w-14 flex-shrink-0">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className={cn("text-lg font-semibold tabular-nums", colorClasses.text)}>
                  {progress}
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-1.5 text-sm">
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-1">
                  <span>Sub</span>
                  <span className="font-mono text-foreground">{childCount}</span>
                </div>
              </div>

              {upcomingDeadline && (
                <div className="text-xs text-orange-600 dark:text-orange-400">
                  Due: {new Date(upcomingDeadline).toLocaleDateString()}
                </div>
              )}
            </div>
          </div>
