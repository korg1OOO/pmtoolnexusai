...
export function VersionHistoryPanel({
    ...
    const handleRestore = async (version: SpreadsheetVersion) => {
        if (await confirm(`Restore version from ${new Date(version.created_at).toLocaleString()}?`, { confirmLabel: 'Restore' })) {
            onRestore(version);
            onOpenChange(false);
        }
    };
    ...
    const handleDelete = async (versionId: string) => {
        if (await confirm('Delete this version?', { confirmLabel: 'Delete', variant: 'destructive' })) {
