...
export function RoleManagementDialog({ open, onOpenChange, projectId }: RoleManagementDialogProps) {
    ...
    const handleDelete = async (roleId: string) => {
        if (await confirm('Are you sure you want to delete this custom role?', { confirmLabel: 'Delete', variant: 'destructive' })) {
            await deleteRole.mutateAsync(roleId);
        }
    };

    const handleReset = async (roleId: string) => {
        if (await confirm('Reset this role to default settings?', { title: 'Reset Role', confirmLabel: 'Reset' })) {
