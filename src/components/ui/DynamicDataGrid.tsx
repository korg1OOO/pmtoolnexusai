...
function EditableCell({ value, columnKey, rowId, type, options, onSave }: EditableCellProps) {
    ...
    const handleStart = () => {
        if (type === 'select' || columnKey === 'id' || columnKey === 'key') return;
        setDraft(value);
        setEditing(true);
    ...
    return (
        <div
            onClick={handleStart}
            className={cn(
                "px-2 py-1.5 min-h-[28px] text-xs cursor-text rounded truncate w-full",
                (columnKey !== 'id' && columnKey !== 'key') ? "hover:bg-primary/5" : ""
            )}
            title={value}
        >
