...
export interface PresentationFolderTreeNode extends PresentationFolder {
  children: PresentationFolderTreeNode[];
  presentationCount: number;
}

export function usePresentationFolders(projectId: string | undefined) {
  ...
  const fetchFolders = useCallback(async () => {
    ...
    try {
      ...
      setFolders(data as PresentationFolder[]);
    } catch (error) {
      console.error('Error fetching presentation folders:', error);
      toast({
        title: 'Error',
        description: 'Failed to load presentation folders',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`presentation-folders-${projectId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'presentation_folders',
  ...
  const buildFolderTree = (presentationCounts: Record<string, number> = {}): PresentationFolderTreeNode[] => {
    const folderMap = new Map<string, PresentationFolderTreeNode>();
    
    folders.forEach(folder => {
      folderMap.set(folder.id, {
        ...folder,
        children: [],
        presentationCount: presentationCounts[folder.id] || 0,
      });
    });

    const rootFolders: PresentationFolderTreeNode[] = [];
  ...
  const createFolder = async (
    ...
    color = 'blue'
  ): Promise<PresentationFolder | null> => {
  ...
  const updateFolder = async (
    id: string,
    updates: Partial<PresentationFolder>
  ): Promise<boolean> => {
    try {
  ...
  const getFolderPath = (folderId: string): PresentationFolder[] => {
