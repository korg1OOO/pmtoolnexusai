/**
 * Tests batch 118: Final wave of deep tests for remaining hooks/services/libs.
 * Plus component imports for more large 0%-covered component files.
 */
...
vi.mock('react', () => ({
    default: { useState: vi.fn((v: any) => [typeof v === 'function' ? v() : v, vi.fn()]), useEffect: vi.fn(), useCallback: vi.fn((f: any) => f), useMemo: vi.fn((f: any) => { try { return f(); } catch { return undefined; } }), useRef: vi.fn(() => ({ current: null })), useContext: vi.fn(() => null), memo: vi.fn((c: any) => c), useReducer: vi.fn(() => [{}, vi.fn()]), forwardRef: vi.fn((c: any) => c), createContext: vi.fn(() => ({ Provider: vi.fn(), Consumer: vi.fn() })) },
...
vi.mock('react-router-dom', () => ({ useNavigate: vi.fn(() => vi.fn()), useParams: vi.fn(() => ({ projectId: 'p1' })), useSearchParams: vi.fn(() => [new URLSearchParams(), vi.fn()]), useLocation: vi.fn(() => ({ pathname: '/', search: '', state: null })), BrowserRouter: vi.fn(({ children }: any) => children), Routes: vi.fn(), Route: vi.fn(), Link: vi.fn(), Outlet: vi.fn() }));
...
describe('Final deep test wave (batch 118)', () => {
    const paths = [
        '@/hooks/useProjectData',
        '@/hooks/useProcurement',
        '@/hooks/useResourceAllocation',
