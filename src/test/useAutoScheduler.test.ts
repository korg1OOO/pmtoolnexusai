...
import { useAutoScheduler, useScheduleOnChange } from '@/hooks/useAutoScheduler';

describe('useAutoScheduler', () => {
    describe('exports', () => {
        it('exports all expected items', () => {
            expect(useAutoScheduler).toBeDefined();
            expect(useScheduleOnChange).toBeDefined();
        });
    });

    describe('useAutoScheduler', () => {
        it('can be rendered without crashing', () => {
            const { result } = renderHook(() => useAutoScheduler(), { wrapper: createWrapper() });
