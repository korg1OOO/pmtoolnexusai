...
import { useCalculateCriticalPath, useCriticalPathAutoUpdate } from '@/hooks/useCriticalPath';

describe('useCriticalPath', () => {
    describe('exports', () => {
        it('exports all expected items', () => {
            expect(useCalculateCriticalPath).toBeDefined();
            expect(useCriticalPathAutoUpdate).toBeDefined();
        });
    });

    describe('useCalculateCriticalPath', () => {
        it('can be rendered without crashing', () => {
            const { result } = renderHook(() => useCalculateCriticalPath(), { wrapper: createWrapper() });
