...
import { useLinkableItems, useAllLinkableItems } from '@/hooks/useLinkableItems';

describe('useLinkableItems', () => {
    describe('exports', () => {
        it('exports all expected items', () => {
            expect(useLinkableItems).toBeDefined();
            expect(useAllLinkableItems).toBeDefined();
        });
    });

    describe('useLinkableItems', () => {
        it('can be rendered without crashing', () => {
            const { result } = renderHook(() => useLinkableItems(), { wrapper: createWrapper() });
