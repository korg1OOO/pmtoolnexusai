...
import { usePlanConfigs, usePlanConfig, useUpdatePlanConfig, formatLimit } from '@/hooks/usePlanConfigs';

describe('usePlanConfigs', () => {
    describe('exports', () => {
        it('exports all expected items', () => {
            expect(usePlanConfigs).toBeDefined();
            expect(usePlanConfig).toBeDefined();
            expect(useUpdatePlanConfig).toBeDefined();
            expect(formatLimit).toBeDefined();
        });
    });

    describe('usePlanConfigs', () => {
        it('can be rendered without crashing', () => {
            const { result } = renderHook(() => usePlanConfigs(), { wrapper: createWrapper() });
